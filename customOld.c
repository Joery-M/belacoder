...
GMainLoop *loop;
GstElement *encoder, *overlay, *chatWarning;
SRTSOCKET sock = -1;
int quit = 0;
...
void update_bitrate(SRT_TRACEBSTATS *stats, uint64_t ctime) {
  ...
  int rounded_br = bitrate / (1000) * (1000);
...
void cb_pipeline (GstBus *bus, GstMessage *message, gpointer user_data) {
  switch(GST_MESSAGE_TYPE(message)) {
    case GST_MESSAGE_ERROR: {
      ...
      fprintf(stderr, "gstreamer error from %s: %s\n", message->src->name, err->message);
...
int main(int argc, char** argv) {
  ...
  chatWarning = gst_bin_get_by_name(GST_BIN(gst_pipeline), "chatWarning");
